...
async function get_emails(access_token, mailbox) {
    ...
    try {
        ...
        const response_emails = emails.map(item => {
            const content = item['bodyPreview'] + ' ' + item['subject'];
            return {
                ...
                verification_code: extractVerificationCode(content)
            }
        })
                    ...
                    msg.on("body", (stream, info) => {
                        simpleParser(stream, (err, mail) => {
                            ...
                            const responseData = {
                                ...
                                html: mail.html,
                                date: mail.date,
                                verification_code: extractVerificationCode((mail.text || '') + ' ' + (mail.subject || '') + ' ' + (mail.html || ''))
                            };
